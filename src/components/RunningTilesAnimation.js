...
/**
 * RunningTilesAnimation — layered celebration
 *
 * Score tiers:
 *   100%  → fireworks + confetti + stars + emoji parade  (PERFECT)
 *   ≥80%  → confetti + stars + emoji parade              (EXCELLENT)
 *   ≥60%  → stars + emoji parade                         (GREAT JOB)
 *   <60%  → emoji parade only                            (KEEP GOING)
 *
 * TIMING:
 *   t=0      tiles flash
 *   t=300    celebration layers start
 *   t=1800   score card pops in
 *   t=5000   onComplete fires (extended for fuller celebration)
 */
...
function FireworkParticle({ x, y, color, angle, speed, delay }) {
  return (
    <div
      className="rta__firework-particle"
      style={{
        left: `${x}%`,
        top: `${y}%`,
        background: color,
...
function ConfettiPiece({ left, color, size, duration, delay, shape }) {
  return (
    <div
      className={`rta__confetti rta__confetti--${shape}`}
      style={{
        left: `${left}%`,
        background: color,
        width: `${size}px`,
...
function EmojiParade({ accuracy }) {
  // Pick parade set based on score tier
...
function RunningTilesAnimation({ score, total, onComplete, speak, milestone }) {
  ...
  useEffect(() => {
    ...
    // Speak result after short delay
    ...
    }, 5000); // extended for fuller celebration
  ...
  return (
    <div className="grammar-overlay">
      ...
      {/* Celebration layers — shown immediately, behind score card */}

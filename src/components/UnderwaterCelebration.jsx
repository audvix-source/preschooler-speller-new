...
function buildReef() {
  ...
  const schoolState = {
    ...
    vx: rand(20, 35) * (Math.random() < 0.5 ? 1 : -1),
    vy: rand(-8, 8),
  };
...
function UnderwaterCelebration({ score, total, onComplete, speak }) {
  ...
  useEffect(() => {
    ...
    // ── Story timeline ────────────────────────────────────────────────────────
    // 0.0s  — reef alive, squid swimming peacefully right-center
    // 0.8s  — shark enters from right
    // 1.5s  — shark catches up, chase begins (both move left together)
    // 3.0s  — squid fires ink, shark nose in cloud
    // 5.0s  — squid slowly escapes left, hides behind big coral
    // 5.0s+ — shark roams confused forever

    const SQUID_START_X  = W * 0.62;
    const SQUID_START_Y  = H * 0.42;
    const BIG_CORAL_X    = 45;  // left side hiding coral
    ...
    const story = {
      ...
      squid: {
        ...
        size: W * 0.20,
        state: 'idle', // idle → chased → inked → escaping → hidden
      ...
      shark: {
        x: W + 140, y: H * 0.40,
        size: W * 0.36,
        state: 'entering', // entering → chasing → confused → roaming
        chaseStartX: 0,
        confuseAngle: 0,
        confuseCenterX: 0, confuseCenterY: 0,
        roamVx: -28, roamVy: 5,
      },
    ...
    function drawFrame(now) {
      ...
      // Camera pans left as action moves left
      if (elapsed > 1.5) {
        st.camTargetX = lerp(0, -30, easeOut((elapsed - 1.5) / 3.0));
      }
      ...
      bgGrad.addColorStop(0,   '#005f87');
      bgGrad.addColorStop(0.3, '#0090b0');
      bgGrad.addColorStop(0.6, '#00a896');
      bgGrad.addColorStop(1,   '#004f60');
      ...
      for (let i = 0; i < 6; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(rx, 0); ctx.lineTo(rx + 70, H * 0.72); ctx.lineTo(rx - 15, H * 0.72);
      ...
      // ── BIG coral on left — squid hiding spot ──
      ctx.save();
      ctx.translate(BIG_CORAL_X, BIG_CORAL_Y);
      const bigSway = Math.sin(now / 1200) * 5;
      ctx.rotate(bigSway * 0.015);
      ...
      ctx.save();
      ctx.translate(BIG_CORAL_X - 20, BIG_CORAL_Y + 10);
      ctx.font = `80px serif`;
      ctx.textAlign = 'center'; ctx.textBaseline = 'bottom';
      ctx.fillText('🌿', 0, 0);
      ctx.restore();
      ...
      for (const t of s.reef.turtles) {
        t.x += t.vx * 0.016;
        t.y += Math.sin(now / 1200 + t.phase) * 0.2;
        if (t.x > W + 60) t.x = -60;
        if (t.x < -60)    t.x = W + 60;
      ...
      for (const f of s.reef.fish) {
        f.x += f.vx * 0.016;
        f.y += Math.sin(now / 750 + f.phase) * 0.3;
        if (f.x > W + 60)  f.x = -60;
        if (f.x < -60)     f.x = W + 60;
      ...
      if (elapsed >= 0.8 && st.shark.state === 'entering') {
        ...
        if (p >= 1) {
          st.shark.state    = 'chasing';
          st.shark.chaseStartX = st.shark.x;
          st.squid.state    = 'chased';
        }
      }

      // Beat 3: Chase — both move left together (1.5–3.0s)
      if (st.squid.state === 'chased' || st.shark.state === 'chasing') {
        const chaseElapsed = elapsed - 1.5;
        if (chaseElapsed >= 0) {
          const speed = 55; // px/s — moderate chase pace
          st.squid.x -= speed * 0.016;
          st.squid.y += Math.sin(now / 400) * 0.5;
          // Shark stays just behind squid
          st.shark.x = st.squid.x + st.squid.size * 1.1;
          st.shark.y = lerp(st.shark.y, st.squid.y, 0.04);
        }
      }

      // Beat 4: Ink fires at 3.0s
      if (elapsed >= 3.0 && !st.inkFired) {
        st.inkFired = true;
        st.ink.active = true;
        st.ink.x = st.squid.x + st.squid.size * 0.3;
        st.ink.y = st.squid.y;
        for (let i = 0; i < 50; i++) {
          const p = makeInkParticle(st.ink.x, st.ink.y);
          p.born = now;
          inkParticles.push(p);
        }
        st.squid.state    = 'escaping';
        st.shark.state    = 'confused';
        st.shark.confuseCenterX = st.shark.x;
        st.shark.confuseCenterY = st.shark.y;
        st.shark.confuseAngle   = 0;
        st.squid.escapeStartX   = st.squid.x;
        st.squid.escapeStartY   = st.squid.y;
      }

      // Beat 5: Squid slowly escapes to big coral (3.0–5.0s)
      ...
      // Beat 6: Shark confused in ink (3.0–5.0s) — nose stays in cloud
      if (st.shark.state === 'confused') {
        st.shark.confuseAngle += 0.020;
        // Small tight circle — keeps nose in ink
       ...
       if (elapsed >= 5.2) {
          st.shark.state  = 'roaming';
          st.shark.roamVx = -45;
          st.shark.roamVy = rand(-10, 10);
        }
      }

      // Beat 7: Shark roams forever lost
      if (st.shark.state === 'roaming') {
        st.shark.x += st.shark.roamVx * 0.016;
        st.shark.y += st.shark.roamVy * 0.016;
      ...
      // ── Draw ink cloud (lingers permanently) ──
      if (st.ink.active) {
        st.ink.radius = Math.min(st.ink.targetRadius, st.ink.radius + 1.4);
        for (const p of inkParticles) {
          if (!p.born) continue;
          const age = (now - p.born) / 1000;
          const px  = p.x + p.vx * age * 0.3;
          const py  = p.y + p.vy * age * 0.3;
          const pg  = ctx.createRadialGradient(px, py, 0, px, py, p.r);
          pg.addColorStop(0, `rgba(15, 5, 35, ${p.a})`);
          pg.addColorStop(1, `rgba(10, 4, 25, 0)`);
      ...
      // ── Draw squid (bright, visible) ──
      if (st.squid.state !== 'hidden') {
        const bobY = st.squid.state === 'idle'
          ? st.squid.y + Math.sin(now / 600) * 4
          : st.squid.y;
        ctx.save();
        ctx.translate(st.squid.x, bobY);
        // Always face left (swimming away)
        if (st.squid.state !== 'idle') ctx.scale(-1, 1);
        // Shake when being chased
        if (st.squid.state === 'chased') ctx.translate(Math.sin(now / 55) * 2, 0);
        ctx.font = `${st.squid.size}px serif`;
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.globalAlpha = 1.0; // full opacity — always visible
        ctx.fillText('🦑', 0, 0);
        ctx.globalAlpha = 1;
        ctx.restore();
      }
...
if (st.shark.x > -200 && st.shark.x < W + 200) {
  ctx.save();
  ctx.translate(st.shark.x, st.shark.y);
  const facingRight = (st.shark.state === 'roaming' && st.shark.roamVx > 0);
  if (!facingRight) ctx.scale(-1, 1); // shark image faces right by default
  if (st.shark.state === 'confused' || st.shark.state === 'roaming') {
    ctx.rotate(Math.sin(now / 160) * 0.18);
  }
  const sw = st.shark.size * 1.4;
  const sh = sw * 0.52; // maintain aspect ratio
  if (st.sharkImage && st.sharkImage.complete) {
    ctx.drawImage(st.sharkImage, -sw / 2, -sh / 2, sw, sh);
  }
  ctx.restore();
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background:'linear-gradient(180deg, #005f87 0%, #0090b0 40%, #007a96 100%)',
      }}>
